...
def filename_to_module_name(
    fullpath,
):  # we need to see __init__, hence we don't use anutils.get_module_name.
...
def resolve(current_module, target_module, level):
    ...
    if (
        level > current_module.count(".") + 1
    ):  # foo.bar.baz -> max level 3, pointing to top level
        raise ValueError(
            "Relative import level {} too large for module name {}".format(
                level, current_module
            )
        )
    base = current_module
...
class ImportVisitor(ast.NodeVisitor):
    ...
    def add_dependency(
        self, target_module
    ):  # source module is always self.current_module
        ...
        for k in range(1, len(modpath) + 1):
            ...
            if (
                possible_init != m
            ):  # will happen when current_module is somepackage.__init__ itself
                self.modules[m].add(possible_init)
                self.logger.debug(
                    "    added possible implicit use of '{}'".format(possible_init)
                )

    def visit_Import(self, node):
        self.logger.debug(
            "{}:{}: Import {}".format(
                self.current_module, node.lineno, [alias.name for alias in node.names]
            )
        )
        for alias in node.names:
            self.add_dependency(
                alias.name
            )  # alias.asname not relevant for our purposes
    ...
    def visit_ImportFrom(self, node):
        ...
        if node.module:
            ...
            if node.level > 0:
                self.logger.debug(
                    "    resolved relative import to '{}'".format(absname)
                )
...
def main():
    ...
    if options.cycles:
        ...
        if not cycles:
            ...
            print(
                "Detected the following import cycles (n_results={}).".format(
                    len(unique_cycles)
                )
            )
            ...
            def stats():
                lengths = [
                    len(x) - 1 for x in unique_cycles
                ...
                def median(lst):
                    ...
                    if n % 2 == 1:
                        ...
                        return (
                            tmp[n // 2 - 1] + tmp[n // 2]
                        ) / 2  # e.g. avg of tmp[4] and tmp[5] if n = 10

                return min(lengths), mean(lengths), median(lengths), max(lengths)

            print(
                "Number of modules in a cycle: min = {}, average = {:0.2g}, median = {:0.2g}, max = {}".format(
                    *stats()
                )
            )
    ...
    if make_graph:
        ...
        graph = pyan.visgraph.VisualGraph.from_visitor(
